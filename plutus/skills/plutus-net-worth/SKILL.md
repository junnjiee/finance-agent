...
- Use `plutus market ticker` for current prices. Run `plutus market ticker --help` if unsure of flags.
- Use `plutus market history` only when the user asks about performance over time. Run `plutus market history --help` if unsure of flags.
- For FX rates, use `plutus market ticker` with Yahoo Finance currency pair symbols (e.g. `--ticker USDSGD=X`).
...
- Batch unique tickers into as few `plutus` calls as possible.
...
- For units-based investment accounts, compute `units * current price` using `plutus market ticker`.
...
- When the user asks how the portfolio performed, use `plutus market history` with the appropriate ticker and period flags for units-based holdings.
